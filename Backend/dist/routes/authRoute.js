...
authRouter.get("/appliedUserByJobId/:appliedUserId", (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        const { appliedUserId } = req.params;
        if (!appliedUserId || isNaN(Number(appliedUserId))) {
            res.status(400).json({ error: "Invalid appliedUserId" });
            return;
        }
        const user = yield prisma.user.findUnique({
            where: { id: Number(appliedUserId) },
            select: {
                name: true,
                email: true,
            },
